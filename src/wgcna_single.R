...
tdata <- read_delim('data/TPM_9-species.tsv', delim='\t')
genes <- tdata$Gene
tdata <- data.frame(tdata %>% select(-Gene))
rownames(tdata) <- genes
meta <- t(tdata) %>% rownames() %>% str_split_fixed(string = ., pattern='_', 5) %>% as_tibble()
colnames(meta) <- c('species1', 'species2', 'rep', 'temp', 'licate')
meta <- meta %>% mutate(species = case_when(licate==''~species1, .default=paste(species1, species2, sep='_'))) %>%
        mutate(temp=case_when(licate==''~str_sub(rep,1,-2), .default=str_sub(temp, 1,-2)))
...
#filter out genes with 0 counts in 95%+ of samples (9467/60832 ~ 85% in bac.camel)
data <- data[,colSums(data>0)/nrow(data)>0.95]
#normalize
data <- log2(data +1)
...
#remove these samples from meta
meta <- meta %>% filter(id %in% rownames(data))
...
MEDissThres <- 0.25 #0.25 camel human rhino; 0.05 rat squirrel
